...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		var CSS = [
			...
			"." + PREFIX + "-fail{display:flex;align-items:center;gap:4px;max-width:100%;padding:1px 2px 1px 8px;",
			"border:1px solid var(--dsw-alias-state-error-primary);border-radius:5px;",
			"color:var(--dsw-alias-state-error-primary);font-size:11px;line-height:18px}",
			"." + PREFIX + "-failText{min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}",
			"." + PREFIX + "-failClose{flex:none;padding:0 6px;font:inherit;font-size:11px;cursor:pointer;",
			"color:inherit;background:transparent;border:none}",
			"." + PREFIX + "-failClose:hover{background:var(--dsw-alias-bg-layer-2)}",
			"." + PREFIX + "-chip{display:inline-flex;align-items:stretch;max-width:300px;overflow:hidden;",
			"border:1px solid var(--dsw-alias-border-l1);border-radius:5px}",
			"." + PREFIX + "-chipClose{flex:none;padding:0 6px;font:inherit;font-size:11px;line-height:20px;cursor:pointer;",
			"color:var(--dsw-alias-label-secondary);background:transparent;border:none;",
			"border-left:1px solid var(--dsw-alias-border-l1)}",
			"." + PREFIX + "-chipClose:hover{color:var(--dsw-alias-label-primary);background:var(--dsw-alias-bg-layer-2)}",
			"." + PREFIX + "-dockBtn{flex:1 1 auto;min-width:0;padding:0 8px;font:inherit;font-size:11px;line-height:20px;",
			"cursor:pointer;color:var(--dsw-alias-label-secondary);background:transparent;border:none;",
		 ...
		 * `inline` — a floating card dock at the bottom-right corner
		...
		/** Disposer for the run-output tab type registration, once bound. */
		var disposeSidebarTab = null;
		/** Disposer for the retry that waits for the sidebar service to appear. */
		var sideBarRetry = null;
		...
		/** Why the most recent run request was refused, shown above the composer. */
		...
		/**
		 * Bind `dsh-better-sidebar`'s service once it exists.
		 *
		 * The service is read at most once per apply(), so arrival order decides
		 * whether Panel works at all: if this bundle materializes before the
		 * sidebar's, the lookup is undefined and the mode stays disabled for the
		 * whole session. `dsh.client.inject` is not load ordering — only edges
		 * listed in `dsh.client.external` order rows — so this retry is what
		 * actually guarantees the bind, degrading for a second instead of a
		 * session when the sidebar arrives late.
		 *
		 * @returns true once the service is bound and the tab type is registered.
		 */
		function bindSidebar(ctx) {
			sidebar = ctx.get("betterSidebar") ?? null;
			if (sidebar === null) return false;
			// Already bound: keep the existing registration instead of mounting twice.
			if (panelAvailable) return true;
			if (!probeSidebar()) return false;
			panelAvailable = true;
			try {
				disposeSidebarTab = mountSidebarTab();
			} catch (error) {
				console.error("dsh-run-button: could not register the run-output tab", error);
			}
			return true;
		}

		/** Retry the bind until the sidebar appears, then stop trying. */
		function bindSidebarWhenReady(ctx) {
			if (bindSidebar(ctx)) return;
			var attempts = 0;
			sideBarRetry = ctx.interval(function () {
				attempts += 1;
				if (!bindSidebar(ctx) && attempts <= 30) return;
				if (sideBarRetry !== null) {
					sideBarRetry();
					sideBarRetry = null;
				}
				if (panelAvailable) notify();
			}, 1000);
		}
		...
		function runCommand(block, language, button) {
			...
			callHost("start", request).then(
				...
				function (error) {
					...
					button.setAttribute(ATTR_BUTTON, "failed");
					button.title = String(error);
					// A red chip alone says nothing about WHY. Surface the refusal so a
					// blocked command is diagnosable without opening devtools.
		...
		function RunStrip(props) {
			...
			for (var i = order.length - 1; i >= 0; i -= 1) {
				...
				if (run === undefined) continue;
				// The chip re-opens the run's output; the × drops the run entirely.
				// Dismissing the last chip hides the whole strip, so it never sits
				// above the composer as permanent noise.
				chips.push(react.createElement(
					"span",
					{ className: PREFIX + "-chip", key: run.runId },
					react.createElement(
						"button",
						{
							type: "button",
			...
			var failure = lastFailure === null
				? null
				: react.createElement(
					"div",
					{ className: PREFIX + "-fail", title: lastFailure },
					react.createElement("span", { className: PREFIX + "-failText" }, "✕ " + lastFailure),
					react.createElement(
						"button",
						{
							type: "button",
							className: PREFIX + "-failClose",
							title: "Dismiss",
							"aria-label": "Dismiss the run error",
							onClick: function () { lastFailure = null; notify(); },
						},
						"×",
					),
				);
		...
		function stop() {
			...
			if (typeof disposeSidebarTab === "function") {
				disposeSidebarTab();
				disposeSidebarTab = null;
			}
		...
		function apply(ctx) {
			...
			try {
				...
				// dock output is the default and nothing else changes.
				bindSidebarWhenReady(ctx);
			...
			ctx.effect(function () {
				return function () {
					...
					// `stop()` owns the sidebar disposers, so both paths unwind the same way.
					stop();
				};
			}, "dsh-run-button: dom teardown");
		}

		// `timer` is a HARD dependency: the late-bind retry calls ctx.interval(),
		// and cordis refuses an undeclared service with
		// 'cannot get property "interval" without inject'.
		var inject = ["slots", "connection", "timer"];
		exports.apply = apply;
