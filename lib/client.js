window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function refreshButtons() {
			...
			for (var i = 0; i < nodes.length; i += 1) {
				...
				var status = run.status === "completed" && run.exitCode !== 0 && run.exitCode !== null
					? "failed"
					: run.status === "killed" ? "stopped" : run.status === "completed" ? "done" : run.status;
		...
		function renderPanel(entry) {
			...
			var statusName = run === undefined ? "running" : (run.status === "completed" ? (run.exitCode === 0 || run.exitCode === null ? "done" : "failed") : run.status === "killed" ? "stopped" : run.status);
		...
		function stop() {
			var styleTag = document.getElementById(PREFIX + "-styles");
