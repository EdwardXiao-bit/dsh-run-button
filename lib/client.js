window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		/** Inline needs no optional plugin, so it is the default surface. */
		...
		var CSS = [
			...
			"." + PREFIX + "-inline{position:fixed;z-index:60;box-sizing:border-box;display:flex;flex-direction:column;",
		...
		var MODE_LABELS = { panel: "Panel", inline: "Inline", none: "Off" };
		var MODE_TITLES = {
			panel: "Output in a bottom-panel tab per run (needs dsh-better-sidebar)",
			inline: "Output in a panel under the code block",
		...
		/* ------------------------------------------------------------------ *
		 * Output surface 2: an inline panel anchored under its code block
		 * ------------------------------------------------------------------ */

		/** Find the code block that started a given run, for panel anchoring. */
		function blockForRun(runId) {
			...
			for (var i = 0; i < banners.length; i += 1) {
				...
				if (attached.get(blockKey(commandOf(block), languageOf(block))) === runId) return block;
			}
			return null;
		}

		function positionInline(runId) {
			var root = inlinePanels.get(runId);
			if (root === undefined) return;
			var anchor = blockForRun(runId);
			if (anchor === null) {
			...
			root.style.left = Math.max(16, Math.min(rect.left, window.innerWidth - width - 16)) + "px";
			root.style.right = "auto";
			var below = rect.bottom + 6;
			if (window.innerHeight - below - 16 < 120) {
		...
		function positionAllInline() {
			var ids = Array.from(inlinePanels.keys());
			for (var i = 0; i < ids.length; i += 1) positionInline(ids[i]);
		}
		...
		function start() {
			...
			// on scroll and need their chips (and inline anchors) re-created.
			window.addEventListener("scroll", scheduleScan, true);
			window.addEventListener("resize", positionAllInline);
			if (mode === "panel") {
		...
		function stop() {
			...
			window.removeEventListener("resize", positionAllInline);
