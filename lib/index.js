...
export function apply(ctx) {
  ...
  const mountViaService = () => {
    ...
    try {
      ...
      ctx.logger?.warn?.(`dsh-run-button: service channel unavailable, mounting a direct route: ${String(error)}`)
      return undefined
    }
  }
  ...
  const mountDirectRoute = () => {
    ...
    if (webServer === undefined || typeof webServer.register !== 'function') {
      ctx.logger?.warn?.('dsh-run-button: no Connection RPC service and no web server; run buttons cannot reach the host')
      return undefined
    }
    ...
    return webServer.register({
      ...
      handler: async (req, res) => {
        // Same trust fence the Connection service applies to its own channels.
